...
const MOVE_SCENE_IN_OUT = 'MOVE_SCENE_IN_OUT';
...
export class MotionLab {
    ...
    moveScene(direction = DIRECTION_LEFT_OUT, callback = () => {}) {
        this.job = {};
        this.job.type = MOVE_SCENE_IN_OUT;
        this.job.container = Props.parent.userData.graphContainer;
        this.job.currentTime = 0.0;
    ...
    continueGrow() {
        this.job.related.forEach((relatedSphere) => {
            relatedSphere.position.copy(relatedSphere.userData.target.clone().multiplyScalar(this.job.currentTime));
    ...
    updateRotation() {
        if (Props.parent) {
            ...
            Props.parent.userData.graphContainer.setRotationFromQuaternion(artistPropsSetRotation);
            Props.parent.userData.graphContainer.children.forEach((obj) => {
                if (obj.userData.textMesh) {
    ...
    positionText(sphere) {
        ...
        let box = new THREE.Box3().setFromObject(sphere.userData.textMesh);
        let halfWidth = (box.max.x - box.min.x) / 2;
        textMeshPos = sphere.userData.textMesh.position;
        radius = sphere.userData.radius;
        halfRadius = radius / 2;
