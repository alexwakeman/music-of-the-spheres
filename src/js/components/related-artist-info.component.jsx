...
export function RelatedArtistInfoComponent({relatedArtist, showUI, showRelated, showExploreButton}) {
	...
	return (
		<div className={showRelatedClass}>
			...
			<div className="genres"><span className="title">Genre similarity:</span> <span className="pill">{relatedArtist.genreSimilarity}%</span></div>
			<div className={showExploreButtonClass}><button onClick={(evt) => Props.sceneInst.exploreSelectedArtist()}>Explore</button></div>
		</div>
	)
}
