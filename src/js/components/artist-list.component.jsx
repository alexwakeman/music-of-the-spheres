...
export class ArtistListComponent extends React.Component {
    constructor({handleSliceExploredArtists}) {
        super();
        this.handleSliceExploredArtists = handleSliceExploredArtists;
    }

    handleGetArtist(evt, artistId, index) {
        evt.preventDefault();
        Props.sceneInst.navigateBackFromList(artistId);
        this.handleSliceExploredArtists(index);
    }

    render() {
        let artists = this.props.visitedArtists.map((artist, index) => {
            ...
            return (
                <div className="artist" key={artist.id}>
                    <a href={href} id={artist.id} className="nav-artist-link"
                       onClick={(event) => { this.handleGetArtist(event, artist.id, index) }}>
